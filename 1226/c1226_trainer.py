...
import os
import csv
from c1226_model import Encoder, Decoder, Discriminator
from c1226_utils import (self_supervised_augmentation, SelfSupervisedLoss,
...
class VAEGANTrainer:
    def __init__(self, device='cuda', latent_dim=20, num_classes=10,
        ...
        self.num_classes = num_classes  # [修改1] 保存 num_classes 供后续使用
        self.save_dir = save_dir
        if not os.path.exists(self.save_dir):
            os.makedirs(self.save_dir, exist_ok=True)

        print(f"初始化 VAE-GAN: 图像尺寸={img_size}x{img_size}, 通道数={input_channels}, 类别数={num_classes}")
    ...
    def train_step(self, x_real, labels):
        ...
        # [修改2] 冗余计算优化：一次性获取 validity, logits 和 features
        # real_features 将用于后续生成器的 Feature Matching Loss，避免再次前向传播
        real_validity, real_logits, real_features = self.discriminator(x_real)
        ...
        # [修改3] 旋转逻辑修复：
        # 1. 原始路径：x_real 是未旋转的，不应传入 rotation_labels (或者传 None)
        z_real, KLD, ssl_out_orig = self.encoder(x_real)
        x_recon, _ = self.decoder(z_real)

        # 2. 增强路径：x_aug 是被旋转过的，必须在这里传入 rotation_labels 让模型学习预测角度
        z_aug, _, ssl_out_aug = self.encoder(x_aug, rotation_labels)
        ...
        # [修改2续] 删除了重复的 _, _, real_features = self.discriminator(x_real)
        # 直接使用上面计算好的 real_features.detach()
        ...
        ssl_output = {
            ...
            **ssl_out_rep,
            # 确保旋转预测结果包含在内 (现在在 ssl_out_aug 中)
            **ssl_out_aug
        }
        ...
        # Feature Matching Loss (使用之前计算并 detach 的 real_features)
    ...
    def save_model(self, path):
        state = {
            ...
            'num_classes': self.num_classes  # 保存类别数是个好习惯
        }
    ...
    def visualize_training(self, test_loader=None):
        ...
        if test_loader:
            ...
            # [修改4] 动态生成类别名称
            # 使用 self.num_classes 而不是硬编码 range(10)
            names = [str(i) for i in range(self.num_classes)]
