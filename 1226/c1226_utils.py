...
class SelfSupervisedLoss(nn.Module):
    ...
    def __init__(self, lambda_rot=1.0, lambda_cont=0.5, lambda_inp=0.7):
        ...
        # [修改5] 图像修复损失: 使用 L1 Loss 替代 BCELoss
        # 理由: 图像修复通常是回归问题，L1 Loss 能生成更清晰的纹理，而 BCELoss 仅适用于概率分布
        self.inpainting_loss = nn.L1Loss()
    ...
    def forward(self, outputs, targets):
        ...
        if 'inpainting_pred' in outputs:
            ...
            # 这里 masked_pred 经过了 Sigmoid (在 Model 中) 是 [0,1]
            # masked_target 也是 [0,1]
            # 使用 L1 Loss 计算两者差异
